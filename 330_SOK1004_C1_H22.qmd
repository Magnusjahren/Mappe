...
as_tibble(df$dataset)
df = as_tibble(df$dataset) %>%
  rename(tid = Tid, verdi = value, var = ContentsCode) 
...
df = df %>%
  mutate(verdi1000 = verdi/1000)
 df %>% filter(tid > 1999 & var=="BNP") %>%
  ggplot(aes(x=tid,y=verdi1000)) +
  geom_line() +
  xlab("år") +
  ylab("BNP i tusen")
...
#df wide slår sammen verdi og BNP. 
...
df_wide <- df_wide %>%
  ...
  mutate(prosBNP = DBNP / LBNP*100)
df_wide <- df_wide %>%
mutate(DMEMOBNP = MEMOBNP - LMEMOBNP) %>%
  mutate(prosMEMOBNP = DMEMOBNP /LMEMOBNP*100)
...
# Besvar oppgave IIc 
df_long <- df_wide %>%
  pivot_longer(c("prosBNP", "prosMEMOBNP"), names_to = "var", values_to = "verdi")
```
...
df_long %>%
  ...
  labs(title = "BNP per inbygger", x="år", y="kr per innbygger")
  
#Vi ser tydelig at BNP faller i 2008 og 2020. I 2008 var det finanskrisen som gir mindre BNP. I 2020 var det covid-19 som forårsaket fall i BNP.
